...
typedef struct
{
    string name;  //"string" from cs50.h
    int votes;
    bool eliminated;
}
candidate;
...
int main(int argc, string argv[])
{
    ...
    voter_count = get_int("Number of voters: ");  //"get_int" from cs50.h
    ...
    for (int i = 0; i < voter_count; i++)
    {
        ...
        {
            string name = get_string("Rank %i: ", j + 1);  //"get_string" from cs50.h
...
void tabulate(void)
{
    ...
    for(int rank = 0; rank < candidate_count; rank++)
    {
        for(int i = 0; i < voter_count; i++)
        {
            ...
            else if(candidates[preferences[i][rank+1]].eliminated == false)
            {
                candidates[preferences[i][rank+1]].votes += 1;
            }
        }
    }
}
